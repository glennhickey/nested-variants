...
input_vcf   <- args[1]
output_file <- args[2]
plot_title  <- if (length(args) >= 3) args[3] else "Chromosome Density of Genotyped Variants"
min_length  <- if (length(args) >= 4) as.numeric(args[4]) else 0
...
vcf_data[, chromosome := sub("_[0-9]+_alt$", "", chrom)]

# Use POS as ref_start; for density binning, ref_length can be 1 (point variant)
vcf_data[, ref_start := pos]
vcf_data[, ref_length := 1L]

# Filter by min_length (of the variant itself via REF/ALT difference)
# For call VCFs this filter is less meaningful, but keep interface consistent
if (min_length > 0) {
  cat("Note: min_length filter applies to ref_length (1 for all point variants in call VCF)\n")
}
...
if (is.null(chrom_lengths)) {
  chrom_lengths <- vcf_data[, .(length = max(pos, na.rm = TRUE)), by = chromosome]
