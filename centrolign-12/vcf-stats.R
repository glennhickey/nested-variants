...
if (per_sample) {
  ...
  if (n_samples == 0) {
    ...
    batch_size <- 50
    for (batch_start in seq(1, n_all_samples, by = batch_size)) {
      batch_end <- min(batch_start + batch_size - 1, n_all_samples)
      ...
      if (n_samples <= 20) {
        ...
        p_ps <- ggplot() +
          ...
          geom_point(data = ps_counts,
                     ...
                     size = 1.5, alpha = 0.8) +
          { if (n_samples <= 10)
              geom_text(data = ps_counts,
                        aes(x = variant_type, y = count, label = sample),
                        position = jitter_pos,
                        size = 2.5, hjust = -0.15, vjust = 0.5, check_overlap = TRUE)
          } +
          scale_fill_manual(values = c("On-reference" = "steelblue", "Off-reference" = "coral"),
                            name = NULL) +
          scale_color_manual(values = c("On-reference" = "steelblue", "Off-reference" = "coral"),
