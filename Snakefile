...
rule vcfeval_per_sample:
    ...
    shell:
        ...
        "       {params.out_dir}/eval-regions.bed"
        "       \"$WORK_TMPDIR/\""
        " && {{ [ -f {params.out_dir}/eval-ref.fa.gz.gzi ]"
        "       && cp {params.out_dir}/eval-ref.fa.gz.gzi \"$WORK_TMPDIR/\" || true; }}"
        " && python3 scripts/vcfcomp.py vcfeval"
        "    --truth $WORK_TMPDIR/call.renamed.vcf.gz"
        "    --calls $WORK_TMPDIR/$(basename {input.dv_vcf})"
        "    --ref $WORK_TMPDIR/eval-ref.fa.gz"
        "    --regions $WORK_TMPDIR/eval-regions.bed"
        "    --out-dir $WORK_TMPDIR"
        "    --threads {threads}"
